...
release LEVEL: release-check
    ...
    awk '
        /^## \[Unreleased\]/ {
            while ((getline line < "/tmp/whetstone_cl_section") > 0) print line
            next
        }
        { print }
    ' CHANGELOG.md > CHANGELOG.md.tmp
    mv CHANGELOG.md.tmp CHANGELOG.md
    rm -f /tmp/whetstone_cl_section
    ...
    HEADROOM_MIN=$(grep -m1 'MIN_VERSION' src/headroom.rs | grep -oP '"[^"]+"' | tr -d '"')
    RTK_MIN=$(grep -m1 'MIN_VERSION' src/rtk.rs | grep -oP '"[^"]+"' | tr -d '"')
